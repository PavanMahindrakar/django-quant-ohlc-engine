...
It does NOT:
- Run strategy
- Handle caching (strategy level)
- Place orders

Session reuse logic added to prevent login on every run.
"""
...
class AngelOneService:
    ...
    - Reuse login session until expiry
    """

    # 🔥 Class-level cached session
    _cached_smart = None
    _token_expiry = None
    ...
    # ---------------------------------------------------------
    # LOGIN (Session Reuse Enabled)
    # ---------------------------------------------------------

    def login(self):
        """
        SmartAPI login with in-memory session reuse.

        - Logs in once
        - Reuses session until expiry
        - Automatically refreshes after expiry
        """

        # ♻️ Reuse existing session if valid
        if (
            AngelOneService._cached_smart and
            AngelOneService._token_expiry and
            timezone.now() < AngelOneService._token_expiry
        ):
            print("♻️ Reusing SmartAPI session")
            self.smart = AngelOneService._cached_smart
            return {"status": True, "message": "Session reused"}

        # 🔐 Fresh login required
        print("🔐 Performing fresh SmartAPI login")
        ...
        if not all([
            ...
            raise ValueError("SmartAPI credentials missing in settings")
        ...
        try:
            ...
            # Cache session
            AngelOneService._cached_smart = self.smart

            # ⚠ SmartAPI token usually valid ~6 hours
            AngelOneService._token_expiry = timezone.now() + timedelta(hours=6)
